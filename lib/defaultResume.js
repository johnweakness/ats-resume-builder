...
export function getMissingResumeFields(data = {}) {
  ...
  if (!hasCompleteEntry(data.experience) && !hasCompleteEntry(data.projects)) {
    ...
    if (hasExperienceEntries && !hasProjectEntries) {
      // User only has an Experience section in progress; don't mention Projects.
      missing.push("Experience (title, role, and at least one bullet)");
    } else if (hasProjectEntries && !hasExperienceEntries) {
      // User only has a Projects section in progress; don't mention Experience.
      missing.push("Projects (title, role, and at least one bullet)");
    } else if (hasExperienceEntries && hasProjectEntries) {
      missing.push("Experience or Projects (title, role, and at least one bullet)");
    } else {
      // Neither section has any entries (in any template) — nudge toward
      // Projects specifically so the message is the same regardless of
      // which template/section the user removed.
      missing.push("Projects (add at least one and fill in title, role, and a bullet)");
    }
  }
