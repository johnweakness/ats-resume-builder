...
export function normalizeResume(raw = {}) {
  ...
  return {
    templateId: raw.templateId || "blank",
    ...
    relevantCoursework: raw.relevantCoursework || "",
    availability: raw.availability || "",
    workStudyStrengths: raw.workStudyStrengths || "",
    careerHighlights: raw.careerHighlights || "",
    skills: raw.skills || "",
  };
}
...
export function getMissingResumeFields(data = {}, { requireExperienceAndProjects = true } = {}) {
  const missing = [];
  ...
  if (requireExperienceAndProjects && hasExperienceEntries && !hasCompleteEntry(data.experience)) {
    missing.push("Experience (title, role, and at least one bullet)");
  }
  if (requireExperienceAndProjects && hasProjectEntries && !hasCompleteEntry(data.projects)) {
